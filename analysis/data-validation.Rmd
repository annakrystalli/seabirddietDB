...
Here is a table of all the duplicates flagged for the combination described above.
...
This table is editable. It would be great if you could review and if there are comments about datapoints you want to flag, please just edit the highlighted `comments` column. Once you are finished, **export the table to csv** using an informative name and send it over to me. You can of course just export the table as is and work with it in your prefered editor (ie excel). Beware that if you refresh the page, all your edits will be lost.


I've pulled a few examples to discuss
...
Note also that although the table lists data for 62 stomachs, in the data set the sample size is 79, which in the papaer in listed as the total number of stomachs examined including empty ones.
```{r}
dupes %>% slice(3:4) %>% 
...
Please use issue [#16](https://github.com/annakrystalli/seabirdPrey/issues/16) to discuss


***

# validation of `prey_taxon` 
#### (esp in relation to `prey_orig_descr`)

I've noticed multiple matches values in `prey_taxon` to individual values in `prey_orig_descr`. `prey_orig_descr` was `Prey.type` in the original dataset, defined as the **"terms used by the study"** and I'm using `prey_taxon` instead of `Prey.species` as it seems more appropriate given prey is not consistently identified to species level. I'm a little confused by some of the current matches (see below).

Here is a table of all the unique combinations of variables `prey_taxon` and `prey_orig_descr` where `prey_taxon` corresponds to more than a single `prey_orig_descr`. I suggest you have a browse through yourselves to get a feel for them. 
...
Is there anything that looks amiss. Tom said that species or lower taxa might also be inferred?
...
Please use issue [#17](https://github.com/annakrystalli/seabirdPrey/issues/17) to discuss


# `prey_size`

```{r}
seabirddiet %>% select(prey_size) %>% filter(!is.na(prey_size)) %>% pull() %>% sort
```



To be usable numerically and to convert to a single coulmn, this column would need some pretty intense parsing and multiple columns. For the mere 100 or so data points, I don't feel it's worth it.

I suggest keeping it, but just as a textual description. Please use issue [#18](https://github.com/annakrystalli/seabirdPrey/issues/18) to discuss
